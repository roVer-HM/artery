...
vanetza::asn1::Denm UseCase::createMessageSkeleton()
{
    ...
    int ret = 0;
    const auto taiTime = countTaiMilliseconds(mService->getTimer()->getTimeFor(dProvider->updated()));
    ...
    message->denm.management.eventPosition.longitude = round(dProvider->longitude(), microdegree) * Longitude_oneMicrodegreeEast;
    message->denm.management.eventPosition.latitude = round(dProvider->latitude(), microdegree) * Latitude_oneMicrodegreeNorth;
    ...
    message->denm.location->eventSpeed->speedValue = std::abs(round(dProvider->speed(), centimeter_per_second)) * SpeedValue_oneCentimeterPerSec;
    ...
    message->denm.location->eventPositionHeading->headingValue = round(dProvider->heading(), decidegree);
